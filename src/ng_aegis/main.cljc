(ns ng_aegis.main
  "Kotodama WASM entrypoint for the Ng_aegis clean-room actor (L4) — Clojure port.
  ...
  facts (`ng_aegis.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ng_aegis")
(def tier "L4")
...
(def entity-specs
  [{:entity "Case"      :plural "cases"      :id-prefix "ngaegis_cas"
    :fields [:number :jurisdiction :status :filedAt] :required [:number :jurisdiction]
    :coerce {}                             :refs {}}
   {:entity "Party"     :plural "parties"    :id-prefix "ngaegis_par"
    :fields [:caseId :role :name]          :required [:role :name]
    :coerce {}                             :refs {:caseId "Case"}}
   {:entity "Document"  :plural "documents" :id-prefix "ngaegis_doc"
    :fields [:caseId :type :contentRef]    :required [:type]
    :coerce {}                             :refs {:caseId "Case"}}
   {:entity "Incident"  :plural "incidents" :id-prefix "ngaegis_inc"
    :fields [:type :jurisdiction :occurredAt] :required [:type :jurisdiction]
    :coerce {}                             :refs {}}
   {:entity "Watchlist" :plural "watchlists" :id-prefix "ngaegis_wat"
    :fields [:name :program :entries]      :required [:name :program]
    :coerce {:entries :int}                :refs {}}
   {:entity "Sanction"  :plural "sanctions" :id-prefix "ngaegis_san"
    :fields [:targetName :program :listedAt] :required [:targetName :program]
    :coerce {}                             :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"ng_aegis.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ng_aegis-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ng_aegis-compat/ready)
